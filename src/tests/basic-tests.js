  ...
  if (score.length > 0) {
    ...
    for (let i = 0; i < strippedInputArray.length; i++) {
      // console.log("now testing the index " + i + ", which is " + strippedInputArray[i])
	      ...
	      if (score[i][0] ===	"ب"	) {score[i][1] = score[i][1] 	*	1.5	; score[i][2]  = ["If a ب is at the start of the term, it is probably not part of the root; otherwise it probably is."	]};																													
	      ...
	      if (score[i][0] ===	"س"	) {score[i][1] = score[i][1] 	/ 	2	; score[i][2]  = [	"A س at the very start of a term may indicate the future tense. If it is not the first letter, but is followed by a ت then it may indicate a Form 10 word."	]};																													
